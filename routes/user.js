...
router.get("/cart", verifyLogin, (req, res) => {
  ...
  mysqlConnection.query(
    ...
    (err, result) => {
      ...
      if (result.length > 0) {
        var sql =
          "select  sum((price)*(quantity)) as total from all_product,cart where cart.productId=all_product.id and userId=?";
        mysqlConnection.query(sql, userID, (err, result) => {
          ...
          var sql =
            "select userId,all_product.id,all_product.Name,all_product.price,cart.quantity from all_product,cart where  cart.productId=all_product.id and userId=?  ";
          mysqlConnection.query(sql, userID, (err, result) => {
            if (err) throw err;
            console.log(result);
...
router.post("/place-order", (req, res) => {
  ...
  mysqlConnection.query(
    ...
    (err, result) => {
      ...
      console.log(result);
      mysqlConnection.query(
        "SELECT LAST_INSERT_ID() AS orderId FROM order_table ",
        (err, result) => {
          if (err) throw err;
...
router.post("/verify-payment", (req, res) => {
  ...
  hmac.update(
    deatails["payment[razorpay_order_id]"] +
      "|" +
      deatails["payment[razorpay_payment_id]"]
  );
  hmac = hmac.digest("hex");
  if (hmac == deatails["payment[razorpay_signature]"]) {
    mysqlConnection.query(
      "update order_table  set status='placed' where id=?",
      orderId,
      (err, result) => {
        if (err) throw err;
        console.log(result);
...
router.get("/cancel-order/:id", (req, res) => {
  ...
  mysqlConnection.query(
    "update order_table set status='Cancelled' where id=? and paymentmethod='COD'",
    orderId,
    (err, result) => {
      if (err) throw err;
      res.redirect("/orders");
